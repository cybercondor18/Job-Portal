...
void jobseeker::saveToCSV_jobseekers() {
    ...
    file << name << "," << email << "," << password<<","<<resume<<","<<this->exp_list[0].job_title<<","<<exp_list[0].company_name<<","<<exp_list[0].duration<<","<<edu_list[0].name_of_grade<<","<<edu_list[0].institute<<","<<edu_list[0].year_dur<<skills[0]<<",";
    file << endl;
    file.close();
}
void loadJobseekers() {
    ...
    while (getline(file, line)) {
        jobseeker jbs;
        stringstream ss(line);
        string name, email, password, resume,job_title,comp_name,job_dur,name_deg,institute,deg_year,skill;
        int year;
        ...
        getline(ss, deg_year, ',');
        getline(ss, skill, ',');
        if (!name.empty() && !email.empty() && !password.empty() && !resume.empty() && !job_title.empty() && !comp_name.empty() && !job_dur.empty() && !name_deg.empty() && !institute.empty() && !deg_year.empty() && !skill.empty()) {
            year=stoll(deg_year);
            ...
            exp.company_name=comp_name;
            exp.duration=job_dur;
            edc.name_of_grade=name_deg;
            edc.institute=institute;
            edc.year_dur=year;
            jbs.skills.push_back(skill);
            jbs.exp_list.push_back(exp);
            jbs.edu_list.push_back(edc);
